...
Optional `codex.model`, `codex.model_provider`, and `codex.reasoning_effort` values select the model and reasoning effort only for App Server turns created or resumed through the IM bridge. The reasoning effort must be one of `none`, `minimal`, `low`, `medium`, `high`, or `xhigh`. The provider remains defined in the user's Codex configuration, and its credential stays in the environment variable named by that provider's `env_key`.
